...
static uintmax_t			ft_recursive_atouimax(const char *str, char **endptr, int base, char *digits, int res)
{
	if (endptr)
		*endptr = (char *)str;
	if (!ft_strchr(digits, ft_tolower(*str)))
		return (res);
	return (ft_recursive_atouimax(str + 1, endptr, base, digits, res * base + ft_strclen(digits, ft_tolower(*str))));
}
