...
static intmax_t			ft_recursive_atoimax(const char *str, char **endptr, int base, char *digits, int res)
{
	if (endptr)
		*endptr = (char *)str;
	if (!ft_strchr(digits, ft_tolower(*str)))
		return (res);
	return (ft_recursive_atoimax(str + 1, endptr, base, digits, res * base + ft_strclen(digits, ft_tolower(*str))));
}
